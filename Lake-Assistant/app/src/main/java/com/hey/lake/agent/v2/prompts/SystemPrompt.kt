package com.hey.lake.agent.v2.prompts

class SystemPrompt(
    private val actionDescription: String,
    private val maxActionsPerStep: Int = 5,
    private val overrideSystemMessage: String? = null,
    private val extendSystemMessage: String? = null
) {

    fun getSystemMessage(): Message {
        val baseMessage = overrideSystemMessage ?: generateBaseMessage()
        val finalMessage = if (extendSystemMessage != null) {
            "$baseMessage\n$extendSystemMessage"
        } else {
            baseMessage
        }
    ...
    private fun generateBaseMessage(): String {
        val template = """
...
{intents_catalog}
        ...
        return template.replace("{max_actions}", maxActionsPerStep.toString())
            .replace("{available_actions}", actionDescription)
    }
}
